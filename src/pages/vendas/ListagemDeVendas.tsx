...
import { IListagemVenda, VendasService } from "../../shared/services/api/vendas/VendasService";
...
export const ListagemDeVendas: React.FC = () => {
  ...
  const [rows, setRows] = useState<IListagemVenda[]>([]);
  ...
  useEffect(() => {
    ...
    debounce(() => {
      VendasService.getAll(pagina, busca)
        .then((result) => {
          setIsLoading(false);
            ...
            {rows?.map(row => (
              <TableRow key={row.id}>
                <TableCell>
                  ...
                  <IconButton size='small' onClick={() => navigate(`/vendas/detalhe/${row.id}`)}>
                    <Icon>edit</Icon>
                  </IconButton>
                </TableCell>
